...
public class BuildTreeByPreIn {
	...
	public static class Solution {
		public TreeNode buildTree(int[] preorder, int[] inorder) {
			...
			// 构建二叉树
			return solve(inorder, 0, inorder.length - 1, preorder, 0, preorder.length - 1);
		}
		 ...
		 * @return 二叉树
		 */
		public TreeNode solve(int[] inorder, int is, int ie, int[] preorder, int ps, int pe) {
			...
			if (is >= 0 && is <= ie && ps >= 0 && ps <= pe) {
				...
				if (is == ie) {
					...
					// 找根结点在中序遍历的下标
					int idx = is;
					while (idx < ie && inorder[idx] != preorder[ps]) {
						idx++;
					}
					...
					if (leftLength > 0) {
						// ps, ps + leftLength - 1，前序遍历的左子树的起始，结束位置
						root.left = solve(inorder, is, idx - 1, preorder, ps + 1, ps + leftLength);
					}

					// 右子树非空，构建右子树
					int rightLength = ie - idx;
					if (rightLength > 0) {
						// ps + leftLength, pe - 1，前序遍历的右子树的起始，结束位置
						root.right = solve(inorder, idx + 1, ie, preorder, ps + leftLength + 1, pe);
					}
